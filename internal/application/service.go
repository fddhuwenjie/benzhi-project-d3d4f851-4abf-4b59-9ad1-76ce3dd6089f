...
type Service struct {
	repo     Repository
	locks    *Coordinator
	packages map[string]domain.InstallationPackage
	now      func() time.Time
}

func NewService(repo Repository) *Service {
	return &Service{repo: repo, locks: NewCoordinator(), packages: map[string]domain.InstallationPackage{}, now: func() time.Time { return time.Now().UTC() }}
}
...
func clonePackage(pkg domain.InstallationPackage) domain.InstallationPackage {
	pkg.CanonicalPayload = append([]byte{}, pkg.CanonicalPayload...)
	return pkg
}
func (s *Service) loadPackage(ctx context.Context, id string) (domain.InstallationPackage, error) {
...
func (s *Service) VerifyPackageDeep(ctx context.Context, id string) (domain.PackageVerification, error) {
	...
	pkg, err := s.loadPackage(ctx, lookupID)
	if errors.Is(err, ErrNotFound) && projectErr == nil && project.Package != nil {
		pkg, err = s.loadPackage(ctx, project.Package.SHA256Digest)
	}
	if err != nil {
