...
func NewService(repo Repository) *Service {
	return &Service{repo: repo, locks: NewCoordinator(), now: func() time.Time { return time.Now().UTC() }}
}
...
func (s *Service) VerifyPackageDeep(ctx context.Context, id string) (domain.PackageVerification, error) {
	...
	var ready, checks sync.WaitGroup
	ready.Add(3)
	checks.Add(3)
	...
	go func() {
		...
		if !domain.VerifyPackage(pkg) {
			...
			report.Summary.Failures = append(report.Summary.Failures, "规范载荷摘要或 package_id 不一致")
		}
	}()
	go func() {
		defer checks.Done()
		...
		if !report.Structure.Passed || !report.Business.Passed {
			report.Summary.Passed = false
			report.Summary.Failures = append(report.Summary.Failures, "规范载荷结构或业务内容不一致")
		}
	}()
	go func() {
		defer checks.Done()
		ready.Done()
		<-start
		if pkg.ProjectID != project.ProjectID || pkg.SourceRevision != project.Revision-1 {
			report.Summary.Passed = false
			report.Summary.Failures = append(report.Summary.Failures, "安装包项目归属或来源修订不一致")
		}
	}()
	ready.Wait()
	close(start)
